...
const constants = __importStar(require("../controller/constants"));
    ...
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            if (!userName || !emailid || !password || !roleId) {
                next(apierr_1.default.badReq("enter valid input"));
            }
            console.log(userName + emailid + password + roleId);
            let emailIdcheckStatus = yield (0, validation_1.emailvalidation)(emailid);
            if (emailIdcheckStatus == false) {
                console.log("email id error");
                next(apierr_1.default.badReq("in valid mail id"));
            }
            let userNameValidation = yield (0, validation_1.AlphaNumberorNot)(userName);
            if (userNameValidation == false) {
                next(apierr_1.default.badReq("Enter User name alpha numaric values only"));
            }
            let userNamelengthValidation = yield (0, validation_1.lengthVerification)(userName, 2, 20);
            if (userNamelengthValidation == false) {
                next(apierr_1.default.badReq("enter the user name in between 2 and 20 characters only"));
            }
            let mailcount = yield user_1.default.find({ emailid: emailid }).count();
            if (mailcount > 0) {
                next(apierr_1.default.badReq(messages.EIexists));
                return;
            }
            let userNameCount = yield user_1.default.find({ userName: userName }).count();
            if (userNameCount > 0) {
                next(apierr_1.default.badReq(messages.UNexists));
                return res;
            }
            const passwordHash = bcrypt.hashSync(password, 10);
            let key = yield (0, validation_1.autogenerateKey)(19);
            ...
            user.save().then(user => {
                ...
                (0, email_1.default)(emailIdformail, emailid, "Admin Portal Registation Successfully", htmlContent, "User registered Successfully", "");
                res.status(200).json({
                    "statusCode": 200,
...
function login(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            yield user_1.default.findOne({ $or: [{ email: userName }, { UserName: userName }] }).then(user => {
                if (user) {
                    // const passwordHash = bcrypt.hashSync(password, 10);
            ...
            console.error(error);
            console.log("==============================error========================");
            next(apierr_1.default.badReq("check the keys"));
        }
    });
}
exports.login = login;
function activate(req, res, next) {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            ...
            let resultforId = yield user_1.default.updateOne(update, active);
            ...
            next(apierr_1.default.badReq("error while activate your account"));
        }
    });
}
